...
const { CONST_TO_FUNCTION } = require('./constants');
const {
  log,
  firstToUpperCase,
  getWordKey,
...
class TsxI18n extends BaseClass {
  ...
  // 节点处理
  compileHandler = (node, nodeType, type, filename) => {
    const handelr = this[CONST_TO_FUNCTION[nodeType]] || (node => node)
    return handelr(node, type, filename);
  }

  // i18n
  i18nHandler = (node, type, filename) => {
    
  }
  // console
  consoleHandler = (node, type, filename) => {
    
  }
  // string
  stringHandler = (node, type, filename) => {
    this.filenameMap[filename].isReplace = true;
  }
  // template
  templateHandler = (node, nodeType, type, filename) => {
    
  }
  // jsx
  i18nHandler = (node, nodeType, type, filename) => {
    
  }

  compile = (type) => {
    Object.keys(this.filenameMap).forEach(filename => {
      this.baseCompile(filename, (node, nodeType) => this.compileHandler(node, nodeType, type, filename));
    })
  }
