...
class TextI18n extends BaseClass {
  ...
  checkTranslate = (filename, codeText) => {
    ...
    if (result) {
      this.setHandler(filename, codeText, {
  ...
  checkColon = (filename, codeText) => {
    if (/(:|：)$/.test(codeText)) {
      this.setHandler(filename, codeText, '需检查替换后是否冒号缺失')
    }
  }

  // 节点处理
  compileHandler = (filename, type, node, nodeType) => {
    const handler = this[CONST_TO_FUNCTION[nodeType]] || (node => node);
    const codeInfo = this.getCodeInfo(node, nodeType);
    
    // 复杂模版字符串，直接返回手动处理
    if (codeInfo.handler) {
      this.setHandler(filename, codeInfo.codeText, codeInfo.handler);
      return node;
    }
    ...
    // 标记需要替换的文件
    if([NODE_TYPE_TEMPLATE, NODE_TYPE_STRING, NODE_TYPE_JSXTEXT].includes(nodeType)){
      this.filenameMap[filename].isReplace = true;
    }

    return handler(node, codeInfo);
  }
  ...
  stringHandler = (node, codeInfo) => {
    let newNode = ts.factory.createIdentifier(`i18n('${findKey(codeInfo.codeText, this.wordMap).key}')`);
    // 组件属性 
    if (ts.isJsxAttribute(node.parent)) {
      newNode = ts.factory.createIdentifier(`{i18n('${findKey(codeInfo.codeText, this.wordMap).key}')}`);
    }
    return newNode;
  }
  // template
  templateHandler = (node, codeInfo) => {
    const code = `i18n('${findKey(codeInfo.codeText, this.wordMap).key}',{${codeInfo.data.join(',')}})`;
    return ts.factory.createIdentifier(code);
  }
  // jsx
  jsxHandler = (node, codeInfo) => {
    return ts.factory.createIdentifier(`{i18n('${findKey(codeInfo.codeText, this.wordMap).key}')}`);
  }
