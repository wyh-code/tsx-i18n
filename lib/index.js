...
const { log, COLLECT, REPLACE, REPLACE_PRE_KEY } = require('./utils');
...
class TsxI18n extends BaseClass {
  ...
  replaceKey = (node, { prevKey, keyStr }, filename) => {
    ...
    if (prevKey) {
      ...
      if (text) {
        const newKeyInfo = this.findKey(text, this.oldWordMap);
  ...
  compile = (type) => {
    Object.keys(this.filenameMap).forEach(filename => {
      ...
      // 替换文案
      if ([REPLACE_PRE_KEY, REPLACE].includes(type)) {
  ...
  writeLog = () => {
    ...
    Object.keys(this.filenameMap).forEach(filename => {
      ...
      if (this.filenameMap[filename].isReplace) {
        ...
        if (!this.notReplaceKeyMap[filename]) {
          console.log(filename, '==notReplaceKeyMap==')
        }
    ...
    log.green(`replaceKeyMap: 已替换的key ${Object.keys(replaceKeyMap).length} 个`)
    log.green(`replaceNotFindKeyMap: 在替换范围，但新字典未找到的key ${Object.keys(replaceNotFindKeyMap).length} 个`)
    log.green(`notReplaceKeyMap: 不在替换范围内的key ${Object.keys(notReplaceKeyMap).length} 个`)
    log.red('------------------------------------------------------------')
