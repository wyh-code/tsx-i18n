    ...
    const visitor = (node) => {
      if (ts.isImportDeclaration(node)) {
        ...
        if (
          ...
          !this.config.exclude(moduleSpecifier) // 排除不需要替换的文件
        ) {
  ...
  replaceTransformer = (type, filename) => (context) => {
    const visitor = (node) => {
      ...
      if (ts.isTemplateLiteral(node) && /.*[\u4e00-\u9fa5]+.*/.test(node.getText())) {
        ...
        if (this.translateError[filename] && this.translateError[filename][newCode]) {
          this.filenameMap[filename].handler = this.filenameMap[filename].handler || {}
        ...
        // 如果以冒号结尾，需人工检查替换后是否冒号缺失
        if (/(:|：)$/.test(newCode)) {
          this.filenameMap[filename].handler = this.filenameMap[filename].handler || {}
          this.filenameMap[filename].handler[newCode] = '需人工检查替换后是否冒号缺失'
        }

        return ts.factory.createIdentifier(`i18n('${this.findKey(newCode)}',{${data.join(',')}})`);
      }
      ...
      if (ts.isStringLiteral(node) && node.text && /.*[\u4e00-\u9fa5]+.*/.test(node.text)) {
        ...
        // 变量声明 - 对象属性 - 中文参数 
        let newNode = ts.factory.createIdentifier(`i18n('${this.findKey(newCode)}')`);

        // 组件属性 
        if (ts.isJsxAttribute(node.parent)) {
          newNode = ts.factory.createIdentifier(`{i18n('${this.findKey(newCode)}')}`);
        }
      ...
      if (ts.isJsxText(node) && node.text && /.*[\u4e00-\u9fa5]+.*/.test(node.text)) {
        ...
        return ts.factory.createIdentifier(`{i18n('${this.findKey(node.text.trim())}')}`);
      }
